	  ...
	  do
	    {
	      if (o_flg)
		{
		  if (fprintf (fo, "%-14.13s: ", pid_ptr->id_name) ==
		      ERROR)
		    lst_err ();
		}
	      else
...
TRACE("nl");
  ...
  if (++paglin == LINES_PER_PAGE)
  ...
  else if (!prt_ref)
    {
      if (o_flg)
	{
	  if (fprintf (fo, "%-4.4d.%4.4d:\t", ++linum, ++edtnum) ==
	      ERROR)
	    lst_err ();
	}
      else
